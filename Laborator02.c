...
struct Telefon* copiazaPrimeleNElemente(struct Telefon* vector/*, int nrElemente*/, int nrElementeCopiate) {
	//copiem intr-un vector nou primele nrElementeCopiate
...
void copiazaTelefoaneScumpe(struct Telefon* vector, char nrElemente, float pretMinim, struct Telefon** vectorNou, int* dimensiune) {
	...
	//determinam pentru cate elemente trebuie sa alocam spatiu
	*dimensiune = 0;
	for (int i = 0; i < nrElemente; i++) {
	...
	//in cazul in care are adresa de dinainte ar fi bine sa il verificam si trebuie sa avem grija sa il dereferentiem de fiecare data
	if ((*vectorNou) != NULL) {
		free(*vectorNou);
	}
...
struct Telefon getPrimulTelefonByProducator(struct Telefon* vector, int nrElemente, const char* producator /*conditie dupa care cautam*/) {
...
int main() {
	...
	//trebuie sa alocam un vector de telefoane
	//ca sa facem asta trebuie sa declaram un pointer la structura telefon si apoi sa ii alocam spatiu

	struct Telefon* telefoane = NULL; //struct Telefon pointer telefoane si fiind vector trebuie sa il initializez cu NULL la inceput
	int nrTelefoane = 3;
